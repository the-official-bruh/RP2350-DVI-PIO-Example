...
parameter:
    ...
    Color_Background : Select the background color
******************************************************************************/
void Draw_Char(uint16_t Xpoint, uint16_t Ypoint, const char Acsii_Char,
                    const font_char* Font, const uint8_t* Font_Glyphs, uint16_t Color_Foreground, uint16_t Color_Background)
{
    uint16_t Page, Column;
    ...
    //uint32_t Char_Offset = (Acsii_Char - ' ') * Font->h * (Font->w / 8 + (Font->w % 8 ? 1 : 0));
    //uint32_t Char_Offset = Font->offset[Acsii_Char]
    uint32_t Char_Offset = Font[Acsii_Char].offset;

    //printf("Char Offset %2d\n", Char_Offset);

    const unsigned char *ptr = &Font_Glyphs[Char_Offset];
    /*
    for (int i = 0; i < 12; i++){
        printf("Char %x\n", *(ptr+i));
    }
    */
    ...
    /*
    uint16_t pix_color_r;
    uint16_t pix_color_g;
    uint16_t pix_color_b;
    uint16_t pix_color;
    uint8_t intensity;
    */
    ...
    for (Page = 0 /*Font[Acsii_Char].top*/; Page < 72; Page++)
    {
        for (Column = 0 /*Font[Acsii_Char].left*/; Column < Font[Acsii_Char].left + Font[Acsii_Char].w; Column++)
        {
            //printf("Char %x\n", *ptr);

            if (Page < Font[Acsii_Char].top){
                //if (alpha == 0){
                    Paint_SetPixel(Xpoint + Column, Ypoint + Page, Color_Background);
                //}
            }
            else if (Column < Font[Acsii_Char].left){
                //if (alpha == 0){
                    Paint_SetPixel(Xpoint + Column, Ypoint + Page, Color_Background);
                //}
            }
            else if (Page > (Font[Acsii_Char].top + Font[Acsii_Char].h) -1){
                //if (alpha == 0){
                    Paint_SetPixel(Xpoint + Column, Ypoint + Page, Color_Background);
                //}
            }
            else if (Column > (Font[Acsii_Char].left + Font[Acsii_Char].w) -1){
                //if (alpha == 0){
                    Paint_SetPixel(Xpoint + Column, Ypoint + Page, Color_Background);
                //}
            }
                ...
                //printf("Char %x\n", *ptr);
                //if (alpha == 0){
                    Paint_SetPixel(Xpoint + Column, Ypoint + Page, Color_Foreground * ((*ptr >> index) & 1));
                //}
                ...
                */
                ...
                if (intensity == 0){
                    ...
                    pix_color_r = ((((Color_Foreground >> 11) & 0b011111) * intensity) / 255) << 11;
                    pix_color_g = ((((Color_Foreground >>  5) & 0b111111) * intensity) / 255) << 5;
                    pix_color_b = ((((Color_Foreground >>  0) & 0b011111) * intensity) / 255) << 0;
                    pix_color = pix_color_r | pix_color_g | pix_color_b;
                
                    printf("color %x\n", pix_color);

                    Paint_SetPixel(Xpoint + Column, Ypoint + Page, pix_color);
                }
                */
...
parameter:
    ...
    Color_Background : Select the background color
******************************************************************************/
void Draw_String(uint16_t Xstart, uint16_t Ystart, const char *pString,
                         const font_char* Font, const uint8_t* Font_Glyphs, uint16_t Color_Foreground, uint16_t Color_Background)
{    
    uint16_t Xpoint = Xstart;
    uint16_t Ypoint = Ystart;
    ...
    while (*pString != '\0')
    {
        ...
        if ((Xpoint + Font[*pString].w) > Paint.Width)
        {
            Xpoint = Xstart;
            Ypoint += Font[*pString].h;
        }

        // If the Y direction is full, reposition to(Xstart, Ystart)
        if ((Ypoint + Font[*pString].h) > Paint.Height)
        {
            Xpoint = Xstart;
            Ypoint = Ystart;
        }
        Draw_Char(Xpoint, Ypoint, *pString, Font, Font_Glyphs, Color_Foreground, Color_Background);

        // The next character of the address
        pString++;

        // The next word of the abscissa increases the font of the broadband
        Xpoint += (Font[*pString].advance);
    }
}
...
void Paint_DrawImage(const unsigned char *image, uint16_t xStart, uint16_t yStart, uint16_t W_Image, uint16_t H_Image)
{
    ...
    for (j = 0; j < H_Image; j++)
    {
        //for (i = 0; i < W_Image/2; i++)
        for (i = 0; i < W_Image; i++)
        {
            const unsigned char *pix = (image + (j*W_Image) + (2*i));

            if (xStart + i < Paint.WidthMemory && yStart + j < Paint.HeightMemory) // Exceeded part does not display
                //Paint_SetPixel(xStart + i, yStart + j, ((uint8_t)*(pix+0) << 8) | ((uint8_t)*(pix+1)));
                //Paint_SetPixel(xStart + i, yStart + j, *(pix));
                Paint_SetPixel(xStart + i, yStart + j, (*(pix+0) << 8) | (*(pix+1)));
                //Paint_SetPixel(xStart + i+1, yStart + j, *(pix+0) << 8 | *(pix+1));
